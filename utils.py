...
class MolDataModule(pl.LightningDataModule):
    def __init__(self, batch_size, file):
        ...
        self.train_data, self.test_data = random_split(
            self.dataset,
            [int(round(len(self.dataset) * 0.8)), int(round(len(self.dataset) * 0.2))],
        )
...
class PropDataModule(pl.LightningDataModule):
    def __init__(self, x, y, batch_size):
        ...
        self.train_data, self.test_data = random_split(
            self.dataset,
            [int(round(len(self.dataset) * 0.9)), int(round(len(self.dataset) * 0.1))],
        )
...
class Dataset(Dataset):
    def __init__(self, file):
        ...
        self.encodings = [
            [self.symbol_to_idx[symbol] for symbol in sf.split_selfies(s)]
            for s in selfies
        ]
    ...
    def __getitem__(self, i):
        return torch.tensor(
            self.encodings[i]
            + [
                self.symbol_to_idx["[nop]"]
                for i in range(self.max_len - len(self.encodings[i]))
            ]
        )
...
def smiles_to_indices(smiles):
    encoding = [
        dm.dataset.symbol_to_idx[symbol]
        for symbol in sf.split_selfies(sf.encoder(smiles))
    ]
    return torch.tensor(
        encoding
        + [
            dm.dataset.symbol_to_idx["[nop]"]
            for i in range(dm.dataset.max_len - len(encoding))
        ]
    )
...
def smiles_to_z(smiles, vae):
    ...
    for i, smile in enumerate(tqdm(smiles)):
        ...
        z = (
            vae.encode(smiles_to_indices(smile).unsqueeze(0).to(device))[0]
            .detach()
            .requires_grad_(True)
        )
...
def one_hots_to_filter(hots):
    ...
    f.close()
    subprocess.run(
        "rd_filters filter --in filter/to_filter.csv --prefix filter/out",
        shell=True,
...
def one_hots_to_affinity(
    hots, autodock, protein_file, num_devices=torch.cuda.device_count()
):
    return smiles_to_affinity(
        [one_hot_to_smiles(hot) for hot in hots],
        autodock,
        protein_file,
...
def smiles_to_affinity(
    smiles, autodock, protein_file, num_devices=torch.cuda.device_count()
):
    ...
    if len(smiles) == 1:
        subprocess.run(
            f"{autodock} -M {protein_file} -s 0 -L ligands/0/ligand0.pdbqt -N outs/ligand0",
            shell=True,
    ...
    for file in tqdm(os.listdir("outs"), desc="extracting binding values"):
        if (
            file.endswith(".dlg")
            and "0.000   0.000   0.000  0.00  0.00" not in open(f"outs/{file}").read()
        ):
...
def one_hot_to_selfies(hot):
    return "".join(
        [
            dm.dataset.idx_to_symbol[idx.item()]
            for idx in hot.view((dm.dataset.max_len, -1)).argmax(1)
        ]
    ).replace(" ", "")
